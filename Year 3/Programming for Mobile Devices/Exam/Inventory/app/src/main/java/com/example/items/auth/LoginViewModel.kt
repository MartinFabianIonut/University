        ...
        viewModelScope.launch {
            ...
            if (result.isSuccess) {
                ...
                uiState = uiState.copy(
                    ...
                    itemIds = result.getOrNull()?.itemIds ?: emptyList(),
                    totalItems = result.getOrNull()?.itemIds?.size ?: 0,
                    downloading = true
                )

                downloadItems(result.getOrNull()?.itemIds ?: emptyList())
                if (uiState.downloadFailed) {
                    Log.d(TAG, "login - downloadItems failed at ${System.currentTimeMillis()}")
