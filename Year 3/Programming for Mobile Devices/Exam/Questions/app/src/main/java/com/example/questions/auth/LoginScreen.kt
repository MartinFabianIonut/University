...
fun LoginScreen(onClose: () -> Unit) {
    ...
    Scaffold(
        ...
        Column(
            ...
            if (loginUiState.downloading) {
                Text(text = "Downloading ${loginUiState.downloadedQuestions + 1}/${loginUiState.totalQuestions}")
            }
            if (loginUiState.downloadFailed) {
                Button(onClick = {
                    GlobalScope.launch {
                        loginViewModel.resumeDownload()
                    }
                }) {
                    Text("Retry")
                }
    ...
    LaunchedEffect(loginUiState.downloading, loginUiState.authenticationCompleted) {
        if (!loginUiState.downloading && loginUiState.authenticationCompleted) {
            Log.d(TAG, "Downloading questions ended")
