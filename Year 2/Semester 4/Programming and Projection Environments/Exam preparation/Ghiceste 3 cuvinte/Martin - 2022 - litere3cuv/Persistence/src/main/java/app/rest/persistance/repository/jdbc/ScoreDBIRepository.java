...
public class ScoreDBIRepository implements IScoreRepository<Integer, Score> {
    ...
    private Score getScoreFromResultSet(ResultSet resultSet) throws SQLException {
        ...
        Integer positionGuessedWord = resultSet.getInt("positionGuessedWord");
        ...
        return new Score(score_id, game, player, round, positionGuessedWord, word, points, totalPoints, localDateTime);
    }
    ...
    public boolean add(Score score) {
        ...
        Connection connection = dbUtils.getConnection();
        try (PreparedStatement ps = connection.prepareStatement("" +
                "insert into score (game_id, player_id, round, positionGuessedWord, word, points, totalPoints, time) " +
                "values (?, ?, ?, ?, ?, ?, ?, ?)")) {
            ps.setInt(1, score.getGame().getId());
            ps.setInt(2, score.getPlayer().getId());
            ps.setInt(3, score.getRound());
            ps.setInt(4, score.getPositionGuessedWord());
            ps.setString(5, score.getWord());
            ps.setInt(6, score.getPoints());
            ps.setInt(7, score.getTotalPoints());
            LocalDateTime localDateTime = score.getTime();
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
            String datetimeString = localDateTime.format(formatter);
            ps.setString(8, datetimeString);
    ...
    public boolean update(Score score) {
        ...
        Connection connection = dbUtils.getConnection();
        try (PreparedStatement ps = connection.prepareStatement("" +
                "update score set game_id = ?, player_id = ?,round = ?," +
                " positionGuessedWord=?, word=?, points=?, totalPoints = ?, time = ? where score_id = ?")) {
            ps.setInt(1, score.getGame().getId());
            ps.setInt(2, score.getPlayer().getId());
            ps.setInt(3, score.getRound());
            ps.setInt(4, score.getPositionGuessedWord());
