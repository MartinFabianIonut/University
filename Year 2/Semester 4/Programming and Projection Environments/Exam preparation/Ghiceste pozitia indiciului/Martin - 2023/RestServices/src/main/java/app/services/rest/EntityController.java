...
public class EntityController {
        ...
        public DTORest[] getAll2(@PathVariable Integer id){
            ...
            for (Score score : scores) {
                ...
                DTORest dto = new DTORest(score.getAttempts(), score.getPositions(), score.getText());
                dtos[i] = dto;
                i++;
            }
            return dtos;
...
//        }
        ...
        public ResponseEntity<?> create(@RequestBody Game game){
            if (game.getCol() < 0 || game.getCol() > 3) {
                return new ResponseEntity<>("Coloana invalida", HttpStatus.BAD_REQUEST);
            }
            if (game.getRow() < 0 || game.getRow() > 3) {
                return new ResponseEntity<>("Linie invalida", HttpStatus.BAD_REQUEST);
            }
