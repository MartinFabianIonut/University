...
public class ServicesRpcProxy implements IService {
    ...
    private void handleUpdate(Response response) {
        ...
        if (response.type() == ResponseType.CONTROL) {
            try {
                GamePlayer gamePlayer = (GamePlayer) response.data();
                employeeObserver.updateControl(gamePlayer);
            } catch (MyException e) {
                logger.error("Error handle update: " + e);
            }
        }
    ...
    public String setControlGameOver(GamePlayer gamePlayer) throws MyException {
        Request req = new Request.Builder().type(RequestType.SET_CONTROL_GAME_OVER).data(gamePlayer).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
