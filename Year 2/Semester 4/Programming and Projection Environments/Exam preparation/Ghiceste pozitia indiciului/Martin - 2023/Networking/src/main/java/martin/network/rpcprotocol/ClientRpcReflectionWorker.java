...
public class ClientRpcReflectionWorker implements Runnable, IObserver {
    ...
    private Response handleSET_CONTROL_GAME_OVER(Request request) {
        ...
        GamePlayer gamePlayer = (GamePlayer) request.data();
        try {
            String control = service.setControlGameOver(gamePlayer);
    ...
    @Override
    public void updateControl(GamePlayer gamePlayer) throws MyException {
        Response resp = new Response.Builder().type(ResponseType.CONTROL).data(gamePlayer).build();
