...
public class GameORMDBIRepository implements IGameRepository<Integer, Game> {
    ...
    public boolean delete(Integer id) {
        ...
        try(Session session = sessionFactory.openSession()){
            ...
            try{
                ...
                Game game = session.createQuery(hql, Game.class)
                        .setParameter("id", id)
                        .setMaxResults(1)
                        .uniqueResult();
                System.out.println("Stergem mesajul "+ game.getId());
                session.delete(game);
                tx.commit();
                ...
                if (tx!=null)
                    tx.rollback();
                logger.error("ERROR for delete in GameORMDBIRepository: " + ex);
            }
        }
        return false;
    }
    ...
    @Override
    public boolean update(Game show) {
