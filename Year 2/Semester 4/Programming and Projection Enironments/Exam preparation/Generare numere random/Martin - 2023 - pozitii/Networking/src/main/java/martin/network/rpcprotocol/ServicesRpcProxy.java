...
public class ServicesRpcProxy implements IService {
    ...
    @Override
    public Game getGame(Player player) throws MyException {
        Request req = new Request.Builder().type(RequestType.GET_GAME).data(player).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
    ...
    public Integer generate(Player player) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException, MyException {
        Request req = new Request.Builder().type(RequestType.GENERATE).data(player).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
