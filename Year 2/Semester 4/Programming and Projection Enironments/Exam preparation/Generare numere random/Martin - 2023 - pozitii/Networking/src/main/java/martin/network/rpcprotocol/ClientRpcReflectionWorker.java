...
public class ClientRpcReflectionWorker implements Runnable, IObserver {
    ...
    private Response handleGET_GAME(Request request) {
        ...
        try {
            Game game = (Game) service.getGame(player);
    ...
    private Response handleGENERATE(Request request) {
        logger.traceEntry("method entered: handleGENERATE_ENEMY with parameters " + request);
        Player player = (Player) request.data();
        try {
            Integer position = service.generate(player);
            logger.info("Enemy generated");
            return new Response.Builder().type(ResponseType.OK).data(position).build();
        } catch (MyException | InvocationTargetException | NoSuchMethodException | IllegalAccessException e) {
            logger.error("Error in worker (solving method handleGENERATE_ENEMY): " + e);
            return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
        }
    }
