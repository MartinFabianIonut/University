...
{
	Prezentare local = NULL;

	local = (Prezentare)malloc(sizeof(struct _Prezentare));
	if (NULL == local)
	{
		return;
	}
...
void ui_copie(Prezentare prezi) {


	Repo copie = Copiere(prezi->srv);
	char* dePrintat = NULL;
	for (int i = 0; i < dimensiune(copie); i++) {
		Medicament medi = get(copie, i);
		GetNume(medi, &dePrintat);
		printf("Id: %d,\t Cod: %d,\t Nume: %s, \t Concentratie: %f, \t Cantitate: %d;\n", GetID(medi), GetCod(medi), dePrintat, GetConcentratie(medi), GetCantitate(medi));
...
id:
	...
	medi = GetMedicamentDupaIDSrv(prezi->srv, id);
	if (medi == NULL) {
...
void ui_afiseaza(Prezentare prezi) {
	...
	int n = 0;
	char* dePrintat = NULL;
	GetToateMedicamenteleSrv(prezi->srv, &toateMedicamentele, &n);
	if (n == 0) {
		printf("\nNu exista medicamente inregistrate!\n");
	}
	for (int i = 0; i < n; i ++){
		GetNume(toateMedicamentele[i], &dePrintat);
		printf("Id: %d,\t Cod: %d,\t Nume: %s, \t Concentratie: %f, \t Cantitate: %d;\n",GetID(toateMedicamentele[i]), GetCod(toateMedicamentele[i]), dePrintat, GetConcentratie(toateMedicamentele[i]), GetCantitate(toateMedicamentele[i]));
		DistrugeMedicament(&toateMedicamentele[i]);
		free(dePrintat);
	}
